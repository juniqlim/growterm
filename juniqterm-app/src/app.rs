        ...
        match event {
            ...
            AppEvent::KeyInput { keycode, characters, modifiers } => {
                ...
                if modifiers.contains(Modifiers::SUPER) {
                    ...
                    if keycode == juniqterm_macos::key_convert::keycode::ANSI_V {
                    ...
                    let zoom_delta = match keycode {
                        kc if kc == juniqterm_macos::key_convert::keycode::ANSI_EQUAL => Some(2.0f32),
                        kc if kc == juniqterm_macos::key_convert::keycode::ANSI_MINUS => Some(-2.0f32),
                        _ => None,
                    };
...
fn render(drawer: &mut GpuDrawer, terminal: &Arc<Mutex<TerminalState>>, preedit: &str, sel: &Selection) {
    ...
    let commands = juniqterm_render_cmd::generate(&visible, cursor, preedit_str, sel_range);
    drop(state);
