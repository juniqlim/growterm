...
fn test_cmd_v_english(mtm: MainThreadMarker) {
    ...
    assert_key_input(&app_event, 0x09, juniqterm_macos::Modifiers::SUPER);
...
fn setup_view(mtm: MainThreadMarker) -> (objc2::rc::Retained<juniqterm_macos::view::TerminalView>, mpsc::Receiver<juniqterm_macos::AppEvent>) {
    let view = juniqterm_macos::test_support::create_terminal_view(mtm);
    let (tx, rx) = mpsc::channel();
    juniqterm_macos::test_support::set_view_sender(&view, tx);
    (view, rx)
}

fn send_perform_key_equivalent(view: &juniqterm_macos::view::TerminalView, event: &NSEvent) -> bool {
...
fn assert_key_input(event: &juniqterm_macos::AppEvent, expected_keycode: u16, expected_mod: juniqterm_macos::Modifiers) {
    match event {
        juniqterm_macos::AppEvent::KeyInput { keycode, modifiers, .. } => {
