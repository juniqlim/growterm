...
fn pty_grid_wait(
    reader: juniqterm_pty::PtyReader,
    cols: u16,
    rows: u16,
    timeout: Duration,
...
fn shell_prompt_appears_on_startup() {
    let (reader, _writer) = juniqterm_pty::spawn(24, 80).expect("failed to spawn PTY");
...
fn echo_command_output_appears_in_grid() {
    ...
    let (reader, mut writer) = juniqterm_pty::spawn(24, 80).expect("failed to spawn PTY");
    ...
    writer
        .write_all(b"echo JUNIQTERM_TEST_OK\n")
        .expect("write failed");
    ...
    while Instant::now() < deadline {
        ...
        match rx.recv_timeout(remaining.min(Duration::from_millis(100))) {
            Ok(data) => {
                ...
                for row in 0..24 {
                    if row_text(&grid, row).contains("JUNIQTERM_TEST_OK") {
                        found = true;
                        break;
                    }
                }
    ...
    assert!(found, "expected 'JUNIQTERM_TEST_OK' in grid after echo command");
}
