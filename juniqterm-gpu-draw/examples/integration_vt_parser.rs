...
use juniqterm_render_cmd::generate;
...
fn build_grid_from_escapes() -> Vec<Vec<Cell>> {
    ...
    let terminal_output = concat!(
        // Row 0: "juniqterm" in bright green, bold
        "\x1b[1;32mjuniqterm\x1b[0m - VT Parser Integration Test\r\n",
        // Row 1: empty
        "\r\n",
...
impl ApplicationHandler for App {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        let attrs = Window::default_attributes()
            .with_title("juniqterm - integration: bytes → VtParser → Grid → generate → draw")
    ...
    fn window_event(&mut self, event_loop: &ActiveEventLoop, _id: WindowId, event: WindowEvent) {
        match event {
            ...
            WindowEvent::RedrawRequested => {
                if let Some(drawer) = &mut self.drawer {
                    let commands = generate(&self.grid, None, None);
