...
자라나는 터미널 앱 — Rust로 만든 GPU 가속 터미널 에뮬레이터. macOS 지원. 몇 가지 재밌는 기능 포함.
...
- **뽀모도로 타이머** — 설정 가능한 작업/휴식 사이클, 입력 차단 (기본 25분/3분), 메뉴에서 리셋
...
AI 코칭, 반투명 모드, 뽀모도로 리셋은 전용 키가 없다. View 메뉴에서 실행한다.

### 리눅스에서

GNOME과 X11이 Super 키를 가져가므로, 단축키는 Ctrl과 Alt 위에 놓인다.
메뉴바도 없어서, 토글들과 뽀모도로 리셋은 독 아이콘 우클릭 메뉴에 있다.
...
unfocused_tint = 0.25                         # 포커스 잃은 창을 회색으로 (0.0 = 끄기)
